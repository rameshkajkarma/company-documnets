...
class NetworkEquipmentService {
  ...
  async getAll(filter: FilterQuery<INetworkEquipment> = {}, page = 1, limit = 10) {
    ...
    const [items, total] = await Promise.all([
      NetworkEquipmentModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean().select("-__v"),
  ...
  async update(id: string, payload: Partial<INetworkEquipment>) {
    return NetworkEquipmentModel.findByIdAndUpdate(id, payload, { new: true, runValidators: true }).lean().select("-__v");
  }
